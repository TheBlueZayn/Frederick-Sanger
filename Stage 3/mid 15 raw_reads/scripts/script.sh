...
# Serotyping using SeqSero2
# Define input directory for read assemblies
input_dir="results/read_assembly/"

# Ensure output directory exists
output_dir="results/seq_sero/"
mkdir -p $output_dir

# Loop through each assembly in the input directory
for assembly in $input_dir/*_assembly/scaffolds.fasta; do
    # Extract sample name from the assembly filename
    sample=$(basename "$assembly" _assembly/scaffolds.fasta)

    # Run SeqSero2 for serotyping using k-mer mode
    if SeqSero2_package.py -m k -t 4 -i "$assembly" -d "${output_dir}${sample}_serotyping"; then
        echo "Finished serotyping for sample: ${sample}."
    else
        echo "Serotyping using SeqSero2 failed for sample: ${sample}"
        exit 1
    fi
done

echo "SeqSero2 serotyping completed."
