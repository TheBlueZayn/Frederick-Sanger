#!/bin/bash
##RUN THE PROVIDED SETUP.SH FILE##
...
#use bowtie2 for indexing and alginment instead of bwa
bowtie2-build reference.fasta reference.indx.fasta
...
for sample in "${samples[@]}"; do
  ...
  bowtie2 -x ref/reference.indx.fasta -1 "filtered_reads/${sample}_R1.trim.fastq" -2 "filtered_reads/${sample}_R2.trim.fastq" -S SAMS/${sample}.sam
